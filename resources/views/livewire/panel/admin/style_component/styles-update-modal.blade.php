<div class="h-full bg-gray-100">
    <div class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
        <div class="relative bg-white p-6 rounded-lg shadow-xl w-full max-w-sm max-h-[90vh] overflow-y-auto">
            ...
            <h2 class="text-lg font-semibold mb-4" style="color: black">Update Style</h2>

            <form wire:submit.prevent="updateStyle" class="overflow-y-auto max-h-[70vh]">
                ...
                @error('name')
                <div class="text-red-600 text-xs pb-1">{{ $message }}</div> @enderror
                ...
                <div class="flex flex-wrap gap-4 mb-4">
                    @foreach($allCategories as $cat)
                        <label class="flex items-center gap-2" style="color: black">
                            <input type="checkbox" wire:model="selectedCategories" value="{{ $cat->name }}"
                                   class="accent-black">
                            {{ $cat->name }}
                        </label>
                    @endforeach
                ...
                <div class="flex gap-4 mb-4" style="color: black">
                    <label class="flex items-center gap-2">
                        <input
                            type="radio"
                            wire:model="is_additional"
                            name="is_additional"
                            value="yes"
                            class="accent-black"
                            id="is-additional-yes"
                        >
                        Yes
                    </label>
                    <label class="flex items-center gap-2">
